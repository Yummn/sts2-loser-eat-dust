...
适配《杀戮尖塔 2》手机版 v0.103.2、v0.110.1 与桌面版 v0.107.1 的无 BaseLib、纯 DLL 模组。
...
- 回到旧节点后，后续走过节点的记录仍然保留；再次经过其中某层时，会用新时间线的首次进入状态覆盖该层旧记录。
- 每个重新经过的节点只覆盖一次，之后的战斗结束、事件完成或保存退出不会继续覆盖新入口状态。
...
- v0.3.1 新增持久化回溯时间线标记：再次经过已有记录层时覆盖旧记录，并用逐节点完成标记保护新的入口快照。
